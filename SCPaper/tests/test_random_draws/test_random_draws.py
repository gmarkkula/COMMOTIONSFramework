...
N_VALS_PER_PARAM = 2
PARAM_ARRAYS = {}
# PARAM_ARRAYS['T_delta'] = np.linspace(10, 60, 8)
...
def run_fit(model_str, parallel):
    ...
    this_fit = sc_fitting.SCPaperParameterSearch(
        ...
        n_repetitions=sc_fitting.N_PROB_SCEN_REPETITIONS, parallel=parallel,
        n_workers=n_workers, verbosity=verbosity, 
        file_name_format='RandomDrawsTest_%s.pkl')
...
if __name__ == "__main__":
    ...
    for noise in (True,):
        for parallel in (False, True):
            print(f'*** noise = {noise}, parallel = {parallel} ***')
            if noise:
                PARAM_ARRAYS['tau_theta'] = np.logspace(np.log10(0.005), np.log10(5), N_VALS_PER_PARAM)
            else:
                PARAM_ARRAYS['tau_theta'] = np.zeros(N_VALS_PER_PARAM)
            run_fit(MODEL, parallel)
