...
class SimpleAgentMixCtrl(commotions_acc_ctrl.AgentWithGoal):

    @staticmethod
    def get_default_parameters(ctrl_type):
        ...
        if ctrl_type is CtrlType.SPEED:
            ...
            params.k_e = 0.1
            params.DeltaT = 0.5 # s
        elif ctrl_type is CtrlType.ACCELERATION:
            params.k_g = 2
            params.k_sc = 2
            params.k_sg = 0.5
            params.k_dv = 0.1
            ...
            raise RuntimeError('Unexpected control type %s.' % ctrl_type)
        params.ctrl_deltas = np.array([-1, -0.5, 0, 0.5, 1]) # m/s or m/s^2 depending on ctrl_type
    ...
    def get_default_shared_parameters():
        params = commotions_acc_ctrl.Parameters()
        params.T_P = PREDICTION_TIME # prediction time
    ...
    def do_action_update(self):
        ...
        for acc_action in self.params.ctrl_deltas:
    ...
    def get_value_of_predicted_state(self, pos, speed, heading, long_acc, \
        ...
        if self.ctrl_type is CtrlType.SPEED:
            value = -self.params.k_g * goal_distance_change_rate \
                - self.params.k_dv * speed ** 2
        elif self.ctrl_type is CtrlType.ACCELERATION:
            value = -self.params.k_g * goal_distance_change_rate \
                - self.params.k_sg * req_acc_to_goal ** 2 \
    ...
    def __init__(self, name, ctrl_type, simulation, initial_state, goal, \
        ...
        if params is None:
            params = self.__class__.get_default_parameters(ctrl_type)
...
if __name__ == "__main__":
    test_simulation = commotions_acc_ctrl.Simulation(0, 20, 0.1)
    agent_A = SimpleAgentMixCtrl( 'P', CtrlType.SPEED, test_simulation, initial_state = \
        commotions_acc_ctrl.KinematicState( pos = np.array((0, -5)), yaw_angle = None, long_speed = 0 ), \
        goal = np.array((0, 5)), \
        plot_color = 'c' )
    agent_B = SimpleAgentMixCtrl( 'C', CtrlType.ACCELERATION, test_simulation, initial_state = \
        commotions_acc_ctrl.KinematicState( pos = np.array((45, 0)), yaw_angle = None, long_speed = 10 ), \
        goal = np.array((-70, 0)), \
        plot_color = 'm' )
    test_simulation.run()
